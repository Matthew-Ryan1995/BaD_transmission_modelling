...
def calculate_peaks(idx_row):
    ...
    new_params["nomask_fear"] = 0.05 * 0.5
...
pp = (gg.ggplot(data=params, mapping=gg.aes(x="R0", y="w2", fill="peaks")) +
      gg.geom_tile() +
      gg.labs(x="Basic reproduction number", y="Fear of disease", fill="Infection waves") +
...
pp.save(filename="phase_plot_peaks.png", width=15, height=15)

params.to_csv("phase_plot_csv.csv")

# %%
max_peaks = np.where(params.peaks == 6)
params.iloc[max_peaks[0]]
