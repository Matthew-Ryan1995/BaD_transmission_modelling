...
Calculate the peaks from an epidemic for different values of the social influence in and out.  Fix fear at 9
...
w1 = a1 = np.arange(0, 0.5, step=0.025)
# a1 = np.arange(0, 10, step=0.5)
# w1 = 0.05 * a1


params = np.array(np.meshgrid(c, p, w1, a1)).reshape(
    4, len(c) * len(p) * len(a1) * len(w1)).T

params = pd.DataFrame(params, columns=["c", "p", "w1", "a1"])
...
if __name__ == "__main__":
    ...
    params.to_csv("phase_plot_maskInfluence_csv_social_explore.csv")

    # %%

    pp = (gg.ggplot(data=params, mapping=gg.aes(x="w1", y="a1", fill="peaks")) +
          gg.geom_tile() +
          gg.labs(x="w1", y="a1", fill="Infection waves") +
    ...
    pp.save(filename="phase_plot_peaks_maskInfluence_social_explore.png",
            width=15, height=15)

    pp = (gg.ggplot(data=params[params.final_size >= 0], mapping=gg.aes(x="w1", y="a1", fill="final_size")) +
          gg.geom_tile() +
          gg.labs(x="w1", y="a1", fill="Final Size") +
    ...
    pp.save(filename="phase_plot_finalSize_maskInfluence_social_explore.png",
            width=15, height=15)
